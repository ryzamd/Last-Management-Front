export const customerStyles = {
  ...
  navigation: {
    container: "w-[75%] flex justify-start items-center mb-4",
  ...
  tableHeader: {
    ...
    createBtn: "flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-space-500 to-space-800 hover:from-space-300 hover:to-space-500 rounded-lg text-white font-grotesk font-bold text-sm transition-all shadow-lg shadow-space-500/20",
  },

  header: {
    container: "flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8",
    title: "text-3xl font-phudu font-bold text-white tracking-wide",
    actions: "flex items-center gap-3 w-full md:w-auto",
    searchBox: "relative flex-1 md:w-64",
    searchInput: "w-full bg-space-900/50 border border-space-500 rounded-lg pl-10 pr-4 py-2 text-sm font-mono text-white placeholder-space-300/50 focus:outline-none focus:border-space-300 transition-colors",
    searchIcon: "absolute left-3 top-2.5 text-space-500",
  ...
  table: {
    ...
    thName: "px-6 py-4 font-bold w-[70%]",
    thStatus: (hasAction: boolean) => `px-6 py-4 font-bold ${hasAction ? "w-[15%]" : "w-[30%]"}`,
  ...
  modal: {
    ...
    deleteBtn: "px-4 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white font-grotesk font-bold transition-colors shadow-lg shadow-red-500/20",
  },

  customSelect: {
    wrapper: "relative w-full",
    trigger: "w-full bg-space-950 border border-space-500 rounded-lg px-4 py-2.5 text-white font-mono focus:border-space-300 outline-none flex justify-between items-center cursor-pointer transition-all hover:border-space-300",
    dropdown: "fixed z-[9999] bg-space-900 border border-space-500 rounded-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200",
    optionsContainer: "max-h-60 overflow-y-auto custom-scrollbar",
    option: (isSelected: boolean) =>
      `px-4 py-2.5 cursor-pointer font-mono text-sm transition-colors hover:bg-space-500 hover:text-white ${
        isSelected ? "bg-space-800 text-white font-bold" : "text-space-300"
      }`,
    icon: (isOpen: boolean) => `text-space-500 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`,
  }
};